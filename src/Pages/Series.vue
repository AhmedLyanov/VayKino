<template>
    <main>
        <div class="main-content">
            <div class="movies">
                <BlockHeader :title="'Сериалы'" :text="false" :link="false" />
...
export default {
    ...
    watch: {
        async moviesPage() {
            let allMovies = await fetchFreeAPI2(`&type=series&limit=40&page=${this.moviesPage}`)
    ...
    async mounted() {
        ...
        this.allMovies = await fetchFreeAPI2(`&type=series&limit=40&page=${this.moviesPage}`)
        this.clearMovies = this.allMovies.filter(movie => {
            if (!movie.genre) {
                return true;
            }
