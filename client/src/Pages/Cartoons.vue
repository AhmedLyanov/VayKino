<template>
    <main>
        <div class="main-content">
            <div class="movies">
                <BlockHeader :title="'Мультфильмы'" :text="false" :link="false" />
...
export default {
    ...
    watch: {
        async moviesPage() {
            let allMovies = await fetchFreeAPI2(`&type=cartoon&limit=40&page=${this.moviesPage}`)
    ...
    async mounted() {
        window.scrollTo(0, 0);
        document.title = 'Мультфильмы'
        this.showEmailMailing()
        this.allMovies = await fetchFreeAPI2(`&type=cartoon&limit=40&page=${this.moviesPage}`)
        this.clearMovies = this.allMovies.filter(movie => {
            if (!movie.genre) {
                return true;
            }
