<template>
    <main>
        <div class="main-content">
            <div class="movies">
                <BlockHeader :title="'Мультсериалы'" :text="false" :link="false" />
...
export default {
    ...
    watch: {
        async moviesPage() {
            let allMovies = await fetchFreeAPI2(`&type=cartoon-series&limit=40&page=${this.moviesPage}`)
    ...
    async mounted() {
        window.scrollTo(0, 0);
        document.title = 'Мультсериалы'
        this.showEmailMailing()
        this.allMovies = await fetchFreeAPI2(`&type=cartoon-series&limit=40&page=${this.moviesPage}`)
        this.clearMovies = this.allMovies.filter(movie => {
            if (!movie.genre) {
                return true;
            }
